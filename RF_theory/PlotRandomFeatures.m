...
(* ::Subsection:: *)
(*Functions to run P/N experiments given regularization*)
(**)


ClearAll[getValueGivenRegLambda];
Options[getValueGivenRegLambda]:={"\[Lambda]"->0.01, "minND"-> -1,"maxND"->2,"NDPoints"-> 50,  "NbPoints"->50, "saveDir"-> None, "Ks"->{1, 2}}
getValueGivenRegLambda[OptionsPattern[]]:= Module[{
\[Lambda]=OptionValue["\[Lambda]"],
minND=OptionValue["minND"],
maxND=OptionValue["maxND"],
NDPoints = OptionValue["NDPoints"],
...
ClearAll[getValueGivenND];
Options[getValueGivenND]:={"ND"-> 1.0, "min\[Lambda]"-> -3,"max\[Lambda]"->1, "\[Lambda]Points"-> 50, "NbPoints"->50, "saveDir"-> None, "Ks"-> {1,2} };
getValueGivenND[OptionsPattern[]]:= Module[{
ND=OptionValue["ND"],
min\[Lambda]=OptionValue["min\[Lambda]"],
max\[Lambda]=OptionValue["max\[Lambda]"],
NbPoints = OptionValue["NbPoints"],
\[Lambda]Points = OptionValue["\[Lambda]Points"],
saveDir=OptionValue["saveDir"],
Ks = OptionValue["Ks"]},
\[Lambda]Vals=N@Subdivide[min\[Lambda],max\[Lambda], \[Lambda]Points-1];
...
(* ::Subsection:: *)
(*Functions to run P/N experiments given N/D and regularization*)


ClearAll[getValueGivenNDandLambda];
Options[getValueGivenNDandLambda]:={"ND" -> 1.0, "\[Lambda]"-> 0.01, "NbPoints"->50, "saveDir"-> None, "Ks"-> {1, 2}};
